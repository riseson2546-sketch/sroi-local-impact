...
const renderProblemsCheckboxes = (title: string, options: { text: string, hasDetail: boolean }[], data: any) => (<div className="mb-4 p-4 border rounded-lg bg-white print-item-block"><h4 className="font-semibold mb-3">{title}</h4><div className="space-y-3">{options.map((opt, i) => { const isChecked = (data?.section1_problems_before || []).includes(opt.text); const detailValue = data?.[`section1_problems_detail_${i}`]; return (<div key={i} className="print-sub-item"><div className="flex items-start space-x-3"><div className={`mt-1 w-5 h-5 r-m border-2 flex items-center justify-center shrink-0 ${isChecked ? 'bg-green-500' : 'bg-white'}`}>{isChecked && <span className="text-white font-bold text-xs">✓</span>}</div><span className={`text-sm ${isChecked ? '' : 'text-gray-500'}`}>{opt.text}</span></div>{isChecked && opt.hasDetail && (<div className="ml-8 mt-1 p-3 bg-blue-50 rounded-md border"><p className="text-sm text-blue-800"><strong>ระบุ:</strong> {detailValue || <span className="text-gray-400">ไม่ได้ระบุ</span>}</p></div>)}</div>);})}</div></div>);
                        ...
                        <div key={num} className="mb-4 p-3 bg-gray-50 rounded-md border print-sub-item">
                            ...
                            <div className="mt-2">
                                ...
                                {section2.section2_applications?.[`app${num}_method_other`] && otherDetail && (
                                    <div className="ml-8 mt-1 p-2 bg-blue-50 text-sm text-blue-700 rounded"><strong>ระบุ:</strong> {otherDetail}</div>
                                )}
                            </div>
                        </div>
                    );
