...
// --- โครงสร้างคำถามและตัวเลือกทั้งหมด (ถูกต้องและสมบูรณ์) ---
...
const CompleteSurveyViewer: React.FC<{ data?: any }> = ({ data = {} }) => {
  ...
  // ลบการสร้างตัวแปร section1, section2, section3 ออกไป เพื่อลดความสับสน
  ...
  return (
    <div className="max-w-6xl mx-auto p-4 md:p-6 bg-gray-100">
      ...
      <div id="printable-area" className="space-y-6">
        <Card className="print-section-card"><CardHeader className="print-card-header"><CardTitle>ข้อมูลผู้ตอบ</CardTitle></CardHeader><CardContent className="p-6"><div className="grid grid-cols-2 gap-4"><div><span className="font-medium">ชื่อ-สกุล:</span> {data.respondent?.name || 'N/A'}</div><div><span className="font-medium">ตำแหน่ง:</span> {data.respondent?.position || 'N/A'}</div><div><span className="font-medium">หน่วยงาน:</span> {data.respondent?.organization || 'N/A'}</div><div><span className="font-medium">วันที่ตอบ:</span> {data.respondent?.survey_date || 'N/A'}</div></div></CardContent></Card>
        ...
        <Card className="print-section-card"><CardHeader className="print-card-header"><CardTitle>ส่วนที่ 1: ผลลัพธ์จากการเข้าร่วมอบรมหลักสูตรนักพัฒนาเมืองระดับสูง (พมส.) ที่เกิดขึ้นจนถึงปัจจุบัน</CardTitle></CardHeader>
          <CardContent className="p-6 space-y-2">
            {renderCheckboxes("1.1 ผลลัพธ์: ด้านองค์ความรู้", knowledgeOutcomes, data.section1_knowledge_outcomes, undefined, false)}
            {renderCheckboxes("1.1 ผลลัพธ์: ด้านการประยุกต์ใช้องค์ความรู้", applicationOutcomes, data.section1_application_outcomes, data.section1_application_other)}
            {renderTextField("1.2 โปรดอธิบายการเปลี่ยนแปลงที่เกิดขึ้นฯ", data.section1_changes_description)}
            
            {/* **** START OF CHANGE **** */}
            {/* เรียกใช้ฟังก์ชันด้วย object `data` ทั้งหมด */}
            {renderProblemsCheckboxes("1.3 ก่อนเข้าร่วมอบรมฯ ภาพรวมในพื้นที่ของท่านมีปัญหาอะไร", problemsBefore, data)}
            {/* **** END OF CHANGE **** */}

            {renderCheckboxes("1.4 องค์ความรู้ที่นำไปใช้ประโยชน์", knowledgeSolutions, data.section1_knowledge_solutions, data.section1_knowledge_solutions_other)}
            {renderRatingScale("ระดับความรู้ก่อนอบรม", data.section1_knowledge_before, 10, <RatingDescription items={["ระดับ 1 : ไม่ได้ใช้ในการแก้ปัญหา ไม่ตระหนักถึงการมีอยู่", "ระดับ 2 : ตระหนัก แต่ไม่ได้นำไปใช้ในการแก้ปัญหา", "ระดับ 3 : นำไปใช้ในการวิเคราะห์ปัญหา หรือคำนึงถึงในงานของตน", "ระดับ 4 : นำไปใช้ในการออกแบบวิธีการแก้ปัญหา (วางแผน/สร้างแนวทาง/อยู่ในขั้นตอนการทำงาน)", "ระดับ 5 : นำไปใช้ในการบรรจุเป็นแผนระดับสำนัก ภายใต้หน่วยงานของตน", "ระดับ 6 : นำไปใช้บรรจุลงในแผนขับเคลื่อนระดับหน่วยงาน/องค์กรของตนเอง", "ระดับ 7 : นำไปใช้ในการขับเคลื่อนเชิงปฏิบัติการในพื้นที่ที่อธิบายผลได้อย่างชัดเจน", "ระดับ 8 : สามารถเผยแพร่ และมีส่วนในการผลักดันแผน/นโยบายของหน่วยงานหรือพื้นที่อื่น ๆ ได้อย่างชัดเจน อธิบายผลได้", "ระดับ 9 : สามารถขยายผล/ต่อยอด การแก้ปัญหาระดับพื้นที่ได้อย่างชัดเจน อธิบายผลได้", "ระดับ 10 : สามารถนำไปกำหนดระดับนโยบายระดับอำเภอ/จังหวัด/ประเทศ"]}/>)}
            {renderRatingScale("ระดับความรู้หลังอบรม", data.section1_knowledge_after, 10)}
            {renderCheckboxes("1.5 การใช้กลไกข้อมูลสารสนเทศฯ", itUsage, data.section1_it_usage, data.section1_it_usage_other)}
            {renderRatingScale("ระดับการช่วยเหลือของกลไกข้อมูลสารสนเทศฯ", data.section1_it_level, 10)}
            {renderCheckboxes("1.6 การใช้กลไกประสานความร่วมมือ", cooperationUsage, data.section1_cooperation_usage, data.section1_cooperation_usage_other)}
            {renderRatingScale("ระดับการช่วยเหลือของกลไกประสานความร่วมมือ", data.section1_cooperation_level, 10)}
            {renderCheckboxes("1.7 การใช้กลไกการระดมทุน", fundingUsage, data.section1_funding_usage, data.section1_funding_usage_other)}
            {renderRatingScale("ระดับการช่วยเหลือของกลไกการระดมทุน", data.section1_funding_level, 10)}
            {renderCheckboxes("1.8 การใช้กลไกวัฒนธรรมและสินทรัพย์ท้องถิ่น", cultureUsage, data.section1_culture_usage, data.section1_culture_usage_other)}
            {renderRatingScale("ระดับการช่วยเหลือของกลไกวัฒนธรรมฯ", data.section1_culture_level, 10)}
            {renderCheckboxes("1.9 การใช้กลไกเศรษฐกิจสีเขียวฯ", greenUsage, data.section1_green_usage, data.section1_green_usage_other)}
            {renderRatingScale("ระดับการช่วยเหลือของกลไกเศรษฐกิจสีเขียวฯ", data.section1_green_level, 10)}
            {renderCheckboxes("1.10 การใช้กลไกการพัฒนาใหม่", newDevUsage, data.section1_new_dev_usage, data.section1_new_dev_usage_other)}
            {renderRatingScale("1.11 ท่านคิดว่าในภาพรวม กลไกการพัฒนาใหม่ ช่วยในการยกระดับการพัฒนาท้องถิ่น/พัฒนาเมืองระดับใด", data.section1_new_dev_level, 10, <RatingDescription items={["ระดับ 1 : ไม่ได้ใช้ประโยชน์ในการยกระดับการพัฒนาท้องถิ่น ไม่ตระหนักถึงการมีอยู่", "ระดับ 2 : ตระหนัก แต่ไม่ได้นำไปใช้ประโยชน์ในการยกระดับการพัฒนาท้องถิ่น", "ระดับ 3 : นำไปใช้ในการวิเคราะห์การยกระดับการพัฒนาท้องถิ่น หรือคำนึงถึงในงานของตน", "ระดับ 4 : นำไปใช้ในการออกแบบการยกระดับการพัฒนาท้องถิ่น (วางแผน/สร้างแนวทาง/อยู่ในขั้นตอนการทำงาน)", "ระดับ 5 : นำไปใช้ในการบรรจุเป็นแผนการยกระดับการพัฒนาท้องถิ่นในระดับสำนัก ภายใต้หน่วยงานของตน", "ระดับ 6 : นำไปใช้บรรจุลงในแผนการยกระดับการพัฒนาท้องถิ่น ขับเคลื่อนระดับหน่วยงาน/องค์กรของตนเอง", "ระดับ 7 : นำแผนการยกระดับการพัฒนาท้องถิ่นไปใช้ในการขับเคลื่อนเชิงปฏิบัติการในพื้นที่ ที่อธิบายผลได้อย่างชัดเจน", "ระดับ 8 : สามารถเผยแพร่ และมีส่วนในการผลักดันแผน/นโยบายของหน่วยงานหรือพื้นที่อื่น ๆ ได้อย่างชัดเจน อธิบายผลได้", "ระดับ 9 : สามารถขยายผล/ต่อยอด การแก้ปัญหาระดับพื้นที่ได้อย่างชัดเจน อธิบายผลได้", "ระดับ 10 : สามารถนำไปกำหนดระดับนโยบายระดับอำเภอ/จังหวัด/ประเทศ"]}/>)}
            {renderCheckboxes("1.12 ปัจจัยที่ส่งผลต่อความสำเร็จ", successFactors, data.section1_success_factors, data.section1_success_factors_other)}
            {renderTextField("1.13 โปรดอธิบายปัจจัยที่ส่งผลต่อความสำเร็จ", data.section1_success_description)}
            {renderRatingScale("1.14 บทบาทของหลักสูตรฯ ที่ก่อให้เกิดการเปลี่ยนแปลง", data.section1_overall_change_level, 10, <RatingDescription items={["ระดับ 1 : เมืองยังคงเป็นรูปแบบดั้งเดิม ไม่มีการเปลี่ยนแปลง", "ระดับ 2 : เริ่มมีการวางแผนพัฒนาเมืองเบื้องต้น / เริ่มมีการวางแผนเรื่องระบบข้อมูล", "ระดับ 3 : มีโครงการพัฒนาเล็ก ๆ แต่ยังไม่มีผลต่อโครงสร้างเมืองโดยรวม", "ระดับ 4 : เริ่มมีการเปลี่ยนแปลงในโครงสร้างพื้นฐานหรือเศรษฐกิจบางส่วน", "ระดับ 5 : เมืองเริ่มมีการขยายตัวในระดับปานกลาง เช่น พื้นที่อยู่อาศัย พื้นที่เศรษฐกิจ หรือสาธารณูปโภคใหม่ ๆ", "ระดับ 6 : เมืองมีการเปลี่ยนแปลงหลายมิติ เช่น การคมนาคมสาธารณะ พื้นที่สีเขียว หรือการฟื้นฟูเมืองเก่า", "ระดับ 7 : เมืองมีระบบบริหารจัดการที่มีประสิทธิภาพ เช่น การใช้ข้อมูลดิจิทัล (Smart City), การมีส่วนร่วมของประชาชนเพิ่มขึ้น", "ระดับ 8 : เมืองกลายเป็นศูนย์กลางด้านเศรษฐกิจหรือวัฒนธรรมระดับภูมิภาค มีโครงการพัฒนาขนาดใหญ่ เช่น TOD หรือเขตเศรษฐกิจพิเศษ", "ระดับ 9 : เมืองพัฒนาในระดับสูง มีความเชื่อมโยงระหว่าง, ใช้แนวคิดยั่งยืน/อัจฉริยะในหลายมิติ", "ระดับ 10 : เมืองเป็นต้นแบบระดับประเทศหรือโลก เช่น เมืองอัจฉริยะที่ใช้เทคโนโลยีและนโยบายที่ยั่งยืนในทุกด้าน"]}/>)}
          </CardContent>
        </Card>

        {/* The rest of the sections (Section 2 and 3) remain the same */}
        <Card className="print-section-card"><CardHeader className="print-card-header"><CardTitle>ส่วนที่ 2: การพัฒนาและการใช้ประโยชน์จากข้อมูล ความรู้ และความร่วมมือระดับประเทศ</CardTitle></CardHeader>
          <CardContent className="p-6 space-y-2">
            {renderCheckboxes("2.1 1) ในการพัฒนาเมือง ได้ใช้ชุดข้อมูลใดบ้างในการพัฒนา", dataTypes, data.section2?.section2_data_types, data.section2?.section2_data_types_other)}
            {renderTextField("2.1 2) ตามที่ท่านระบุในข้อ 1) ชุดข้อมูลที่ใช้นั้นได้ใช้งานจากแหล่งที่มาใด หรือชุดข้อมูลที่ได้จากการสนับสนุนทุนจากโครงการ", data.section2?.section2_data_sources)}
            {renderCheckboxes("2.1 3) ในการจัดทำชุดข้อมูลหรือฐานข้อมูลในการพัฒนานั้น มีหน่วยงานใดเข้ามาร่วมบ้าง", partnerOrgs, data.section2?.section2_partner_organizations, data.section2?.section2_partner_organizations_other)}
            {renderTextField("2.1 4) ตามที่ท่านระบุในข้อ 3) หน่วยงานดังกล่าวเข้าร่วมอย่างไร", data.section2?.section2_partner_participation)}
            {renderCheckboxes("2.2 ชุดข้อมูลเมืองที่พัฒนาขึ้นสามารถตอบโจทย์และแก้ปัญหาของหน่วยงานของท่านได้ในประเด็นใด", dataBenefits, data.section2?.section2_data_benefits, undefined, false)}
            {renderRatingScale("2.3 ชุดข้อมูลเมืองสามารถตอบโจทย์และแก้ปัญหาของหน่วยงานได้ในระดับใด", data.section2?.section2_data_level, 10, <RatingDescription items={["ระดับ 1 : มีการจัดเก็บเอกสารในรูป Digital แต่ยังไม่ได้ดำเนินการทั้งหมด", "ระดับ 2 : มีระบบจัดเก็บข้อมูลบางส่วนในรูปแบบดิจิทัล แต่ยังไม่มีการเชื่อมโยงหรือใช้จริงในการแก้ปัญหา", "ระดับ 3 : เริ่มมีการใช้ข้อมูลดิจิทัลในการทำงานบางมิติ แต่ยังจำกัดเฉพาะภายในหน่วยงาน", "ระดับ 4 : ข้อมูลถูกจัดเก็บเป็นระบบ และบางส่วนมีการใช้ในการรายงานหรือวิเคราะห์เชิงปฏิบัติการ แต่ยังไม่รองรับการตัดสินใจเชิงนโยบาย", "ระดับ 5 : หน่วยงานสามารถใช้ชุดข้อมูลเพื่อวิเคราะห์ และติดตามปัญหาได้ในระดับปฏิบัติการ เช่น การจัดการขยะ การจัดการภัยพิบัติ", "ระดับ 6 : มีการเชื่อมโยงข้อมูลระหว่างหน่วยงาน เพื่อใช้สนับสนุนการตัดสินใจในระดับโครงการหรือพื้นที่", "ระดับ 7 : ระบบรองรับการทำงานแบบบูรณาการ เช่น มีแดชบอร์ดวิเคราะห์สถานการณ์ร่วม ใช้ร่วมกับภาคประชาชน หรือหน่วยงานเครือข่ายได้", "ระดับ 8 : ชุดข้อมูลสามารถคาดการณ์แนวโน้ม และช่วยออกแบบนโยบายเชิงรุก เช่น การคาดการณ์ภัยพิบัติ", "ระดับ 9 : ชุดข้อมูลมีความสามารถเชิงวิเคราะห์สูง ใช้ปัญญาประดิษฐ์ (Artificial Intelligence : AI) หรือฐานข้อมูลขนาดใหญ่ (Big Data) วิเคราะห์ปัญหาที่ซับซ้อนได้แบบเรียลทา", "ระดับ 10 : ชุดข้อมูลกลายเป็นเครื่องมือหลักในการบริหารเมืองแบบ Smart Governance และขยายผลได้ระดับประเทศ หรือสากล"]}/>)}
            {renderTextField("2.4 ปัจจุบันการพัฒนาชุดข้อมูลเพื่อการพัฒนาเมืองของท่าน ยังได้มีการพัฒนาอย่างต่อเนื่อง หรือ ไม่ หากได้ ข้อมูลใดที่ได้มีการจัดหาเพิ่มเติม หรือข้อมูลใดที่ยังต้องการ แต่ยังไม่มี", data.section2?.section2_continued_development)}
            <div className="p-4 border rounded-lg bg-white print-item-block"><h4 className="font-semibold mb-3">2.5 องค์กรของท่านมีแอปพลิเคชัน (Application) ใด ในการพัฒนาเมืองหรือไม่</h4>{[1, 2].map(num => {const appName = data.section2?.section2_applications?.[`app${num}_name`]; if (!appName || !appName.trim()) return null; const otherDetail = data.section2?.section2_applications?.[`app${num}_method_other_detail`]; const methods = [{key:'buy',label:'ซื้อ'},{key:'develop',label:'องค์กรพัฒนาขึ้นเอง'},{key:'transfer',label:'องค์กรอื่นได้มาถ่ายทอดเทคโนโลยีให้'},{key:'other',label:'อื่น ๆ'}]; return(<div key={num} className="mb-4 p-3 bg-gray-50 rounded-md border print-sub-item"><strong>{num}) ชื่อแอปพลิเคชัน:</strong> {appName}<div className="mt-2"><p className="text-sm font-semibold">• วิธีการได้มา:</p><div className="flex flex-wrap gap-x-4 gap-y-1 ml-4">{methods.map(method => { const isChecked = data.section2?.section2_applications?.[`app${num}_method_${method.key}`]; return (<div key={method.key} className="flex items-center space-x-1.5"><div className={`w-4 h-4 border-2 r-m flex items-center justify-center ${isChecked ? 'bg-black' : ''}`} /><span>{method.label}</span></div>);})}{data.section2?.section2_applications?.[`app${num}_method_other`] && otherDetail && (<div className="w-full mt-1 p-2 bg-blue-50 text-sm text-blue-700 rounded"><strong>ระบุ:</strong> {otherDetail}</div>)}</div></div></div>);})}</div>
            <div className="p-4 border rounded-lg bg-white print-item-block"><h4 className="font-semibold mb-3">2.6 ในปัจจุบันองค์กรของท่านได้มีการขยายเครือข่ายความร่วมมือไปยังหน่วยงานใดบ้าง และเป็นความร่วมมือในด้านใด</h4><div className="space-y-2"><div className="grid grid-cols-2 gap-4 font-medium text-center"><div className="p-2 bg-gray-100 rounded">หน่วยงาน</div><div className="p-2 bg-gray-100 rounded">ด้านความร่วมมือ</div></div>{[...Array(5)].map((_, i) => {const org = data.section2?.section2_network_expansion?.[`org${i+1}`]; const coop = data.section2?.section2_network_expansion?.[`cooperation${i+1}`]; if (!org) return null; return (<div key={i} className="grid grid-cols-2 gap-4 text-sm border-b pb-2"><div className="p-2">{org}</div><div className="p-2">{coop || 'N/A'}</div></div>);})}</div></div>
          </CardContent>
        </Card>
        ...
        <Card className="print-section-card"><CardHeader className="print-card-header"><CardTitle>ส่วนที่ 3: การขับเคลื่อนระบบข้อมูลเมืองหรือโครงการนวัตกรรมต่อยอดสู่การเป็นองค์กร ที่ขับเคลื่อนด้วยข้อมูล (Data Driven Organization)</CardTitle></CardHeader>
          <CardContent className="p-6 space-y-4">
            {section3Factors.map(cat => (<div key={cat.category} className="print-item-block mb-6"><h3 className="text-lg font-bold mb-2 text-purple-700 print-category-heading">{cat.category}</h3><div className="space-y-3">{cat.items.map(item => (<div key={item.field} className="print-sub-item">{renderRatingScale(item.title, data.section3?.[item.field], 5)}</div>))}</div></div>))}
          </CardContent>
        </Card>
      </div>
    </div>
