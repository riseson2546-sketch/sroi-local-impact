...
const renderProblemsCheckboxes = (title: string, options: { text: string, hasDetail: boolean }[], allData: any) => {
    ...
    // เพิ่มการ debug เพื่อดูโครงสร้างข้อมูล
    console.log("=== DEBUG: Problems Before Data Structure ===");
    console.log("allData:", allData);
    console.log("allData.section1:", allData.section1);
        ...
        <div className="mb-4 p-4 border rounded-lg bg-white print-item-block">
            ...
            <div className="space-y-3">
                {options.map((opt, i) => { 
                    ...
                    // ตรวจสอบรายละเอียดจากหลายตำแหน่งที่เป็นไปได้
                    const detailValue = 
                        allData?.[`section1_problems_detail_${i}`] ||           // ระดับบนสุด
                        allData?.section1?.[`section1_problems_detail_${i}`] || // ใน section1
                        allData?.data?.[`section1_problems_detail_${i}`] ||     // ใน data
                        null;
                    
                    // Debug log สำหรับแต่ละข้อ
                    console.log(`Problem ${i} (${opt.text.substring(0, 30)}...):`);
                    console.log(`  - isChecked: ${isChecked}`);
                    console.log(`  - detailValue: "${detailValue}"`);
                    console.log(`  - Checking locations:`);
                    console.log(`    - allData[section1_problems_detail_${i}]: "${allData?.[`section1_problems_detail_${i}`]}"`);
                    console.log(`    - allData.section1[section1_problems_detail_${i}]: "${allData?.section1?.[`section1_problems_detail_${i}`]}"`);
                    console.log(`    - allData.data[section1_problems_detail_${i}]: "${allData?.data?.[`section1_problems_detail_${i}`]}"`);
                                ...
                                <div className="ml-8 mt-1 p-3 bg-blue-50 rounded-md border border-blue-200">
                                    ...
                                    {process.env.NODE_ENV === 'development' && (
                                        <div className="mt-2 p-2 bg-yellow-50 border border-yellow-200 rounded text-xs">
                                            ...
                                            <p>Field: section1_problems_detail_{i}</p>
                                            <p>Found in: {
                                                allData?.[`section1_problems_detail_${i}`] ? 'allData (top level)' :
                                                allData?.section1?.[`section1_problems_detail_${i}`] ? 'allData.section1' :
                                                allData?.data?.[`section1_problems_detail_${i}`] ? 'allData.data' :
                                                'Not found'
                                            }</p>
                                        </div>
                                    )}
