...
// **** START OF CHANGE ****
// A função completa e correta que estava faltando.
const renderProblemsCheckboxes = (title: string, options: { text: string; hasDetail: boolean }[], dataForSection: any) => {
    // Garante que selectedValues seja sempre um array, mesmo se dataForSection.section1_problems_before for null ou undefined.
    const selectedValues = Array.isArray(dataForSection.section1_problems_before) ? dataForSection.section1_problems_before : [];
        ...
        <div className="mb-4 p-4 border rounded-lg bg-white print-item-block">
            ...
            <div className="space-y-3">
                {options.map((opt, i) => {
                    ...
                    // Lê o valor do detalhe diretamente do objeto de dados da seção
                    const detailValue = dataForSection[`section1_problems_detail_${i}`];
                    return (
                        <div key={i} className="print-sub-item">
                            <div className="flex items-start space-x-3">
                                <div className={`mt-1 w-5 h-5 rounded-md border-2 flex items-center justify-center shrink-0 ${isChecked ? 'bg-green-500 border-green-600' : 'bg-white border-gray-300'}`}>
                                    {isChecked && <span className="text-white font-bold text-xs">✓</span>}
                                </div>
                                <span className={`text-sm ${isChecked ? '' : 'text-gray-500'}`}>{opt.text}</span>
                            </div>
                            {/* Mostra o detalhe apenas se o checkbox estiver marcado E o detalhe tiver um valor */}
      ...
      <div id="printable-area" className="space-y-6">
        ...
        <Card className="print-section-card">
          ...
          <CardContent className="p-6 space-y-4">
            ...
            {/* **** A chamada agora usa a função correta e passa os dados corretos **** */}
        ...
        {/* Renderização das Seções 2 e 3 (o código aqui já está correto e permanece o mesmo) */}
        {/*... o restante do seu JSX para as seções 2 e 3 vai aqui ... */}
         <Card className="print-section-card">
