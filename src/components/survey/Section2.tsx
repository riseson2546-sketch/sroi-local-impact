...
const Section2: React.FC<Section2Props> = ({ data, onSave, isLoading = false, onNextSection, onPrevSection }) => {
  ...
  const renderApplicationSection = (appNumber: 1 | 2) => (<div className="space-y-4 p-4 border rounded-lg"><Label className="font-medium">{appNumber}) ชื่อแอปพลิเคชัน (Application)</Label><Input placeholder="ระบุชื่อ" value={formData.section2_applications?.[`app${appNumber}_name`] || ''} onChange={e => handleNestedInputChange('section2_applications', `app${appNumber}_name`, e.target.value)} /><div className="space-y-2"><Label className="font-medium">• วิธีการได้มา</Label>{[{key:'buy',label:'ซื้อ'},{key:'develop',label:'องค์กรพัฒนาขึ้นเอง'},{key:'transfer',label:'องค์กรอื่นได้มาถ่ายทอดเทคโนโลยีให้'}].map(m => (<div key={m.key} className="flex items-center space-x-2"><Checkbox id={`app${appNumber}-${m.key}`} checked={!!formData.section2_applications?.[`app${appNumber}_method_${m.key}`]} onCheckedChange={c => handleNestedInputChange('section2_applications', `app${appNumber}_method_${m.key}`, !!c)} /><Label htmlFor={`app${appNumber}-${m.key}`} className="font-normal cursor-pointer">{m.label}</Label></div>))}</div></div>);
  ...
  const renderStepContent = () => {
    switch (currentStep) {
      ...
      case 3: return (<Card><CardHeader><CardTitle>2.4 ปัจจุบันการพัฒนาชุดข้อมูลเพื่อการพัฒนาเมืองของท่าน ยังได้มีการพัฒนาอย่างต่อเนื่อง หรือ ไม่ <span className="text-red-600">*</span></CardTitle></CardHeader><CardContent><Label>หากได้ ข้อมูลใดที่ได้มีการจัดหาเพิ่มเติม หรือข้อมูลใดที่ยังต้องการ แต่ยังไม่มี</Label><Textarea value={formData.section2_continued_development || ''} onChange={e => handleInputChange('section2_continued_development', e.target.value)} rows={5} /></CardContent></Card>);
